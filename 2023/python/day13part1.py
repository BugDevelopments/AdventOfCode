...
def isSymmetric(M,r,axis=0):
  M = M.swapaxes(0,axis)
  assert(0<r<M.shape[1])
  m=min(M.shape[1]-r,r)
  return (np.fliplr(M[:,r-m:r]) == M[:,r:r+m]).all()


def getSymmetryNumber(M):
  for r in range(1,M.shape[1]):
    if isSymmetric(M,r):
      return r
  
  for r in range(1,M.shape[0]):
    if isSymmetric(M,r,axis=1):
      return 100*r
...
I.extend([len(lines)])
print(sum(getSymmetryNumber(makeBinaryMatrix(lines[i+1:j]))   for i,j in zip(I[0:], I[1:])))